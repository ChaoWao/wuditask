...
WudiTask 是供人类与 AI agent 共同使用的分布式任务队列。GitHub
Issue/PR 是交付进展的事实源；独立 Task Hub 保存执行租约、跨仓依赖、验收与
归档证据。它没有常驻服务器、数据库或包管理器依赖。

## 系统边界

- **工具仓库**：保存 Python CLI、十一个 agent skills、schema、dashboard 源码与测试。
...
- **责任人与租约**：GitHub assignee 表示责任人；WudiTask `claim` 只表示独占
  执行租约，不再保存第二套 `owner`。
...
wuditask tool repository/
  ...
  schemas/task.schema.json           公开任务 v2 契约
  schemas/deletion-receipt.schema.json 公开删除回执 v1 契约

wuditask-hub repository/
  hub.json                           task schema v2 / tool API v3 契约
...
2. 在用户 cache 中初始化或复用 Hub bare repository，并在隔离 worktree 中
   校验 `hub.json` 与任务数据。
3. 校验固定的十一项 WudiTask skill，并把它们链接到 `~/.agents/skills` 与
...
| 对照 GitHub 与队列状态 | `$wuditask-reconcile` | `/wuditask-reconcile` |
...
canonical 描述。`$wuditask-add` 或 `/wuditask-add` 依次复用/创建业务仓
Issue/PR；业务仓无法承载时，在配置的 Hub 建 fallback Issue；两边都不能承载
时才使用带原因的 text source。`links` 只保存辅助资料。
...
wuditask dep-check
wuditask reconcile WDT-20260711T120000Z-A1B2C3
```
...
dashboard，并查询 canonical GitHub source 生成独立 delivery state；任务提交
不会运行工具测试。要发布 Pages，在 Hub 仓 Settings >
...
`unavailable`，领取与 done 归档则 fail closed。

未设置该变量时，private 实践仓仍会完成校验和构建，但跳过 Pages 上传与部署，不产生错误的红色 workflow。

私有仓库是否能启用 Pages 取决于 GitHub 方案。个人 Pro、Team 或 Enterprise 通常可以从私有仓库发布 Pages；GitHub Free 的个人 private 仓不能启用。“源仓库私有”也不代表“站点私有”，一般站点仍然公开。只有具备相应 Enterprise Cloud 组织访问控制时，才应把 Pages 当作受限站点。snapshot 会包含 source URL、assignee、PR 作者/URL、review/check 摘要和查询错误；初次演练必须使用脱敏任务。
