...
Hub schema v3 / tool API v5 的 task 文件只保存 repo、source、creator login、
...
acceptance 只在 source。archive done 实时确认 Issue completed 或 PR merged，并
要求 dependencies ready 和至少一条 evidence。若存在 active agents，任何 outcome
都要求 caller 的 matching login/run_id，active done 还要求 live owner；一个 Hub
commit 把 open 文件移动到 archive、清空全集，并把所有 login/run_id 保存为
completion participants。若没有 active agent，所有 outcome 都必须省略 run_id，
且只有 authenticated `created_by` 可以归档；participants 为空。该路径允许在
WudiTask execute 之外完成的成功交付以及未认领或已 release 的终态工作结束生命
周期，同时不允许其他 login 借终态关闭任务。旧 run_id 会被拒绝而不是忽略。
...
- 无 active agent 的任何 archive 只允许 task creator 且不接受 run_id；
- distributed mutation 拒绝 `--actor` 与 `WUDITASK_ACTOR`，只信任 gh login；
...
v3/API5 是无兼容 cutover。API5 放宽 creator-only、无 active-agent 的成功归档，
但不改变 task v3 字段。Hub 必须在一个 commit 中更新 manifest 与 Pages pinned
tool SHA；现有 task 无需重写。旧工具随后应拒绝 Hub，而不是猜测新授权语义。
